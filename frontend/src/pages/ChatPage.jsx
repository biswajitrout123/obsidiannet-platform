...
const BASE_URL = import.meta.env.MODE === "development" ? "http://localhost:5000" : "https://obsidiannet-backend.onrender.com";
...
export default function ChatPage() {
    ...
    useEffect(() => {
        ...
        // ✅ FIXED: Pass userId directly in the connection handshake
        ...
        const fetchTargetUser = async () => {
            try {
                // ✅ FIXED: Added withCredentials to avoid 401 Unauthorized
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            // ✅ FIXED: Only calling backend API. The backend controller handles Socket.io emission now.
            const res = await axios.post(`${BASE_URL}/api/messages/${targetUserId}`, { text: newMessage }, { withCredentials: true });
                    ...
                    {messages.map((msg, idx) => {
                        ...
                        return (
                            <div key={idx} className={`flex w-full ${isMe ? "justify-end" : "justify-start"}`}>
                                ...
                                <div className={`max-w-[70%] p-3 rounded-lg text-sm ${isMe ? "bg-blue-600 text-white rounded-br-none" : "bg-[#38434d] text-gray-200 rounded-bl-none"
                                    }`}>
                                    {msg.text}
                                </div>
                            </div>
